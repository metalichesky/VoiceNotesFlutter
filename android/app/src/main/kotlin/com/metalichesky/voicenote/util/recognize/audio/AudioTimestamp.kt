 ...
 */
internal class AudioTimestamp(
    private val mByteRate: Int
) {
    companion object {
        fun bytesToUs(bytes: Long, byteRate: Int): Long {
            return ((1000000.0 * bytes.toDouble()) / byteRate).roundToLong()
        }

        fun bytesToMillis(bytes: Long, byteRate: Int): Long {
            return ((1000.0 * bytes.toDouble()) / byteRate).roundToLong()
        }
    }
    ...
    fun increaseUs(readBytes: Int): Long {
        val bufferDurationUs = bytesToUs(readBytes.toLong(), mByteRate)
    ...
    fun getGapCount(frameBytes: Int): Int {
        ...
        val durationUs = bytesToUs(frameBytes.toLong(), mByteRate)
