...
import com.github.olga_yakovleva.rhvoice.worker.ConfirmNetworkDataWorker;
import com.github.olga_yakovleva.rhvoice.worker.DataSyncWorker;
import com.github.olga_yakovleva.rhvoice.worker.NetworkDataSyncWorker;
...
public abstract class DataPack {
    ...
    public boolean install(Context context, IDataSyncCallback callback) {
        ...
        try {
            ...
            while ((entry = inStream.getNextEntry()) != null) {
                ...
                if(BuildConfig.DEBUG) {
                    Log.v(TAG, "Extracting " + entry.getName() + " from " + getName() + " to " + outObj.getAbsolutePath());
                }
    ...
    public final void scheduleSync(Context context, boolean replace) {
        ...
        OneTimeWorkRequest localRequest = (new OneTimeWorkRequest.Builder(DataSyncWorker.class))
                .addTag(Data.WORK_TAG)
                .setInputData(setWorkInput(new androidx.work.Data.Builder()).build())
                .build();
        ...
        if (flag == SyncFlags.NETWORK) {
            cont = cont.then((new OneTimeWorkRequest.Builder(ConfirmNetworkDataWorker.class))
                    .addTag(Data.WORK_TAG).build());
            cont = cont.then((new OneTimeWorkRequest.Builder(NetworkDataSyncWorker.class))
                    .addTag(Data.WORK_TAG)
                    .setConstraints((new Constraints.Builder())
                            .setRequiredNetworkType(getNetworkTypeSetting(context))
                            .build()
                    ).build());
        }
        cont.enqueue();
